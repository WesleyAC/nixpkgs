...
stdenv.mkDerivation rec {
  ...
  nativeBuildInputs = [ nix patchelf autoPatchelfHook makeWrapper ];
  ...
  postUnpack = ''
    ...
    local expected_hash="${ovftoolSystem.postUnpackHash}"
    echo "Expected hash: $expected_hash" >&2
    local ovftool_hash
    ovftool_hash="$(nix-hash --type sha256 --base32 .)"
    echo "Actual hash:   $ovftool_hash" >&2
    if [ "$ovftool_hash" != "$expected_hash" ]; then
